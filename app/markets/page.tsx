...
export default function MarketsPage() {
  ...
  // Shared prop builder so the line (rows) and grid (cards) views render the
  // exact same on-chain data — only the layout differs.
  const itemProps = (s: Stock, i: number): AssetViewProps => ({
    stock: s,
    index: i,
    ...
    derivedBorrowApr: stats.derived ? stats.derived.borrowAprBps / 100 : null,
    derivedVaultApr: stats.derived ? stats.derived.supplyAprBps / 100 : null,
    collateralVolumeUsd: stockAddress(s.sym)
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="max-w-[1320px] w-full mx-auto px-4 sm:px-8 pt-5 pb-3">
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {/* View mode: Line (table rows) ↔ Grid (cards) */}
            <div className="flex gap-0.5 bg-paper-alt border border-hairline-soft rounded-[2px] p-[3px]">
              {([
                ["line", "Line"],
                ["grid", "Grid"],
                ...
                <button
                  ...
                  title={`${label} view`}
                  className="border-0 rounded-[2px] transition-colors font-mono inline-flex items-center gap-1.5"
                  style={{
                    padding: "5px 9px",
                    fontSize: 11,
                    background: view === id ? "var(--ink)" : "transparent",
                    color: view === id ? "var(--paper)" : "var(--ink-soft)",
                ...
                >
                  {id === "grid" ? (
                    <svg width="11" height="11" viewBox="0 0 11 11" fill="currentColor" aria-hidden="true">
                      ...
                      <rect x="6.5" y="0" width="4.5" height="4.5" rx="0.5" />
                      <rect x="0" y="6.5" width="4.5" height="4.5" rx="0.5" />
      ...
      <div className="max-w-[1320px] w-full mx-auto px-4 sm:px-8 flex-1 flex flex-col">
        {stocks.length === 0 ? (
          ...
          /* ── Grid view: one card per asset ──────────────────── */
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 py-4">
            {stocks.map((s, i) => (
              <LedgerCard key={s.sym} {...itemProps(s, i)} />
            ))}
          </div>
        ) : (
          /* ── Line view: ledger table rows ───────────────────── */
          <div className="overflow-x-auto">
            <div style={{ minWidth: 900 }}>
              ...
              <div className="flex-1">
                {stocks.map((s, i) => (
                  <LedgerRow key={s.sym} {...itemProps(s, i)} />
                ))}
              </div>
            </div>
          </div>
...
/* ── Grid view card (same data as LedgerRow, card layout) ────────── */
...
function LedgerCard({
  ...
  return (
    <div
      ...
      className="flex flex-col gap-3.5 border rounded-[2px] bg-paper p-4 cursor-pointer transition-colors duration-150"
      style={{ borderColor: hovered ? "var(--ink)" : "var(--hairline)" }}
    >
      {/* Header: logo + symbol/badge + 24h */}
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 border border-ink rounded-[2px] flex items-center justify-center bg-paper relative overflow-hidden shrink-0">
            <div
              className="absolute inset-0"
              style={{
                animation: `ef-breathe ${breathePeriod}s ease-in-out infinite`,
            ...
            />
            <AssetLogo sym={stock.sym} size={24} className="relative" />
          </div>
          <div className="min-w-0">
            ...
            <div
              className="text-ink-soft mt-0.5 truncate"
              style={{ fontSize: 11 }}
            >
              {stock.name} · <span className="text-ink-mute">{stock.sector}</span>
            </div>
        ...
        <div
          className="font-mono tabular font-medium shrink-0"
          style={{ fontSize: 12, color: up ? "var(--up)" : "var(--down)" }}
          title={
            isReal24h
              ? "24h change from Pyth historical anchor (now vs t-86400s)"
              : "Reference change — Pyth has no historical print for this anchor"
      ...
      <div>
        <div
          key={live.dir + "-" + live.value.toFixed(2)}
          className={`font-serif font-medium tabular inline-block rounded-[2px] ${
            live.dir > 0
              ? "animate-tick-up"
              : live.dir < 0
                ? "animate-tick-down"
      ...
      {/* Stats grid */}
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 border-t border-hairline-soft pt-3">
        <div>
          <div
          ...
          <div
            className="font-mono tabular font-medium mt-0.5"
            style={{ fontSize: 13 }}
            title={
              ltvIsLive
                ? "Max LTV from vault.assets(token).ltvBps"
          ...
          <div className="h-[3px] bg-hairline-soft mt-1 relative">
            <div
              className="absolute left-0 top-0 bottom-0 bg-ink"
              style={{ width: `${liveLtv * 100}%` }}
            />
        ...
        <div>
          ...
          <div className="text-ink-mute mt-0.5" style={{ fontSize: 9 }}>
            collateral locked
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div
            className="font-mono tabular font-medium mt-0.5 text-up"
            style={{ fontSize: 13 }}
          >
            {fmt.signedPct(derivedVaultApr ?? 0, 2)}
          </div>
      ...
      {onChain ? (
        <button
          ...
          className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 border border-ink rounded-[2px] transition-all duration-150 font-medium w-full"
          style={{
            fontSize: 12,
            background: hovered ? "var(--ink)" : "transparent",
            color: hovered ? "var(--paper)" : "var(--ink)",
        ...
        <span
          className="inline-flex items-center justify-center px-3.5 py-2 border border-hairline rounded-[2px] font-medium text-ink-mute w-full"
          style={{ fontSize: 12, cursor: "not-allowed" }}
        >
          Soon
        </span>
